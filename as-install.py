...
def query_yes_no(question, default="yes"):
    ...
    while True:
        ...
        if default is not None and choice == '':
            ...
            sys.stdout.write("{0}Please respond with 'yes' or 'no' (or 'y' or\
...
def main():
    ...
    while True:
        ...
        options = strt_menu.keys()
        options.sort()
        print_command(HEADER)
        for k in options:
            print_info('{0}) {1}'.format(k, strt_menu[k]))

        choice = raw_input('''{0}> Enter the number of your choice: \
{1}'''.format(COLORS['OKBLUE'], COLORS['ENDC']))
        if choice == "1":
            start()
        elif choice == "99":
...
def menu():  # noqa
    ...
    while True:
        ...
        options.sort()
        for k in options:
            print_info('{0}) {1}'.format(k, menu[k]))

        step = raw_input("> Option: ")
        if step == "1":
            check_uefi()
        elif step == "2":
            set_keymap()
...
def start():
    ...
    if not internet_on():
        print_error('''Looks like you're not connected to the internet. \
Exiting.''')
        sys.exit()
        print_info("Checks done. The installation process will now begin.")
        time.sleep(3)
    if os.geteuid() != 0:
...
def check_uefi():
    ...
    try:
        ...
        # Dir doesnt exist
        print_title('''Your computer doesnt seem to be running a UEFI board.\
Continuing..''')
        uefi = False
        set_keymap()
...
is that correct?'''.format(layout), 'yes'):
            system("setxkbmap {0}".format(layout))
        if query_yes_no('''> Try typing in here to test. \
If some characters are coming up different, \
delete it all and type 'Y': ''', 'no'):
...
def partition_menu():
    ...
    while True:
        ...
        for k in options:
            print_info('{0}) {1}'.format(k, menu[k]))
        part = raw_input("{0}> Choice: {1}".format(COLORS['OKBLUE'],
                                                   COLORS['ENDC']))
        try:
            if int(part) in range(1, 4):
                part_type = int(part)
                logger.log(logging.INFO, "Partition Menu: \
                           {0}".format(part_type))
                break
        except:
            print_error("Invalid Option")
...
awk '{print $1}' ''').split('\n')
    drive = raw_input('''{0}> Please choose the drive you would like to\
install ArchStrike on \
(default: /dev/sda ): {1}'''.format(COLORS['OKBLUE'], COLORS['ENDC'])) \
        or '/dev/sda'
    if drive not in available_drives:
        print_error("That drive does not exist")
        time.sleep(1)
        identify_devices()
    if not query_yes_no('''{0}Are you sure want to use {1}? \
Choosing the wrong drive may have very bad \
consequences!'''.format(COLORS['WARNING'], drive), 'yes'):
        identify_devices()
    drive_size = system_output('''lsblk -p | grep -w %s | awk '{print $4}' | \
grep -o '[0-9]*' | awk 'NR==1' ''' % (drive))
...
def partition_devices():
    ...
    while True:
        ...
        options.sort()
        for k in options:
            print_info('{0}) {1}'.format(k, types[k]))

        try:
            fsc = raw_input('''{0}> Choice (Default is ext4): \
{1}'''.format(COLORS['OKBLUE'], COLORS['ENDC'])).strip() or 1
...
def setup_swap():
    ...
    if query_yes_no("> Step 5) Would you like to create a new swap space?",
                    'yes'):
        swap_space = raw_input('''{0}> Enter your swap space size\
(default is 512M): {1}'''.format(COLORS['OKBLUE'], COLORS['ENDC'])
                               ).rstrip() or '512M'
        size = swap_space[:-1]
        if swap_space[-1] == "M":
...
def manual_partition():
    ...
    time.sleep(3)
    print_info('''I'm now going to print the current partition scheme of your \
drive %s''' % drive)
    print_info("But first let's confirm everything.")
    confirm_drive = raw_input('''{0}> Please confirm the drive by typing \
{1}: {2}'''.format(COLORS['OKBLUE'], drive, COLORS['ENDC']))
    if confirm_drive != drive:
        print_error('''That doesn't look right. Let's try identifying those\
again.''')
        identify_devices()
    system("lsblk {0}".format(drive))
    partition_table = system_output('''fdisk -l {0} | grep Disklabel | \
cut -d ' ' -f 3'''.format(drive))
    if partition_table == 'gpt':
        print_info("""
     For the GPT partition table, the suggested partition scheme looks like\
this:
     mountpoint        partition        partition type            boot flag\
suggested size
     ______________________________________________________________________\
___________________
     /boot              /dev/sdx1        EFI System Partition      Yes     \
260-512 MiB

     [SWAP]             /dev/sdx2        Linux swap                No      \
More than 512 MiB

     /                  /dev/sdx3        Linux (ext4)              No      \
Remainder of the device

    WARNING: If dual-booting with an existing installation of Windows on a \
UEFI/GPT system,
    avoid reformatting the UEFI partition, as this includes the Windows .efi\
file required to boot it.
        """)
    elif partition_table == 'dos':
        print_info("""
    For the MBR partition table, the suggested partition scheme looks like \
this:
    mountpoint        partition        partition type            boot flag \
suggested size
    _______________________________________________________________________\
__________________
    [SWAP]            /dev/sdx1        Linux swap                No        \
More than 512 MiB
    /                 /dev/sdx2        Linux (ext4)              Yes       \
Remainder of the device
    """)
    if query_yes_no('''> I've read this and wish to continue to the \
partitioner.''', 'yes'):
        sp.call("clear", shell=True)
        sp.call('cfdisk {0}'.format(drive), shell=True)
        sp.call("clear", shell=True)
        sp.call('lsblk {0}'.format(drive), shell=True)
        if query_yes_no('''> Are you sure your partitions are set up \
correctly?''', 'yes'):
            format_partitions()
        else:
            manual_partition()
...
def format_partitions():
    ...
    partitions = raw_input('''{0}> Enter all the partitions you created by \
seperating them with a comma (e.g. /dev/sda1,/dev/sda2): \
{1}'''.format(COLORS['OKBLUE'], COLORS['ENDC'])).split(',')
    logger.log(logging.INFO, "System Partitions: {0}".format(partitions))
    print_info('''You sure these are the partitions?\n\
{0}'''.format('\n'.join(partitions)))
    if query_yes_no("> ", 'yes'):
        print_info("Alright, starting to format.")
        for i in partitions:
            print_warning("Partition {0} will be formatted now".format(i))
            partition_type = raw_input('''{0}> Enter the partition type \
(linux, uefi, swap): {1}'''.format(COLORS['OKBLUE'], COLORS['ENDC'])).lower()
...
def mount_partitions(partitions):
    ...
    print_info('\n'.join(partitions))
    root = raw_input('''{0}Which one is your / mounted partition? (e.g. \
/dev/sda1): {1}'''.format(COLORS['OKBLUE'], COLORS['ENDC']))
    ...
    if root in partitions:
        ...
        mount_partitions(partitions)
    if partition_table == 'gpt':
        boot = raw_input('''{0}Which one is your /boot mounted partition? \
(e.g. /dev/sda2): {1}'''.format(COLORS['OKBLUE'], COLORS['ENDC']))
...
def check_lvm():
    ...
    if part_type == 1:
        system("clear")
        if query_yes_no('''{0}Automatic partitioning wipes your drive clean \
before proceeding. Are you sure you want to \
continue?'''.format(COLORS['WARNING']), 'no'):
            auto_partition()
        else:
            partition_menu()
    elif part_type == 2:
...
\n\n\nw\ny" | gdisk {1}'''.format(swap_space, drive))
                SWAP = system_output('''lsblk | grep %s | awk '{ if (NR==4) \
print substr ($1,3) }' ''' % (drive[-3:]))
...
gdisk {0}'''.format(drive))
            BOOT = system_output(''' lsblk | grep %s |  awk '{ if (NR==2) \
print substr ($1,3) }' ''' % (drive[-3:]))
            ROOT = system_output(''' lsblk | grep %s |  awk '{ if (NR==3) \
print substr ($1,3) }' ''' % (drive[-3:]))
        else:
            if swap_space != 'None':
                system('''echo -e "o\ny\nn\n1\n\n+100M\n\nn\n2\n\n+1M\nEF02\nn\
\n4\n\n+{0}\n8200\nn\n3\n\n\n\nw\ny" | gdisk {1}'''.format(swap_space, drive))
                SWAP = system_output('''lsblk | grep %s |  awk '{ if (NR==5) \
print substr ($1,3) }' ''' % (drive[-3:])).rstrip()
                system("wipefs -afq /dev/{0}".format(SWAP))
                system("mkswap /dev/{0}".format(SWAP))
                system("swapon /dev/{0}".format(SWAP))
            else:
...
\n3\n\n\n\nw\ny" | gdisk {0}'''.format(drive))
            BOOT = system_output(''' lsblk | grep %s |  awk '{ if (NR==2) \
print substr ($1,3) }' ''' % (drive[-3:]))
            ROOT = system_output(''' lsblk | grep %s |  awk '{ if (NR==4) \
print substr ($1,3) }' ''' % (drive[-3:]))
    else:
        if swap_space != 'None':
            system('''echo -e "o\nn\np\n1\n\n+100M\nn\np\n3\n\n+{0}\nt\n\n82\nn\
...
print substr ($1,3) }' ''' % (drive[-3:]))
            ...
            system('''echo -e "o\ny\nn\n1\n\n+100M\n\nn\n2\n\n+1M\n\
EF02\nn\n3\n\n\n\nw\ny" | gdisk {0}'''.format(drive))
            BOOT = system_output(''' lsblk | grep %s |  awk '{ if (NR==4) \
print substr ($1,3) }' ''' % (drive[-3:]))
            ROOT = system_output(''' lsblk | grep %s |  awk '{ if (NR==2) \
print substr ($1,3) }' ''' % (drive[-3:]))
    else:
        system('''echo -e "o\nn\np\n1\n\n+100M\nn\np\n2\n\n\nw" | \
fdisk {0}'''.format(drive))
...
print substr ($1,3) }' ''' % (drive[-3:]))
    ...
    system('''printf {0} | cryptsetup luksFormat -c aes-xts-plain64 -s 512 \
/dev/lvm/lvroot -'''.format(passwd))
    system('''printf {0} | cryptsetup open --type luks /dev/lvm/lvroot \
root -'''.format(passwd))
    del passwd
...
def install_base():  # noqa
    ...
    while True:
        ...
        choice = raw_input('''{0}> Choice (Default is Arch-Linux-Base): \
{1}'''.format(COLORS['OKBLUE'], COLORS['ENDC'])) or 1
        try:
...
def locale_and_time():
    ...
    while True:
        ...
        choice = raw_input('''{0}> Enter the number for your locale or leave empty \
for default: {1}'''.format(COLORS['OKBLUE'], COLORS['ENDC'])) or '1'
...
Afterwards type in the locale you want to use.''')
            ...
            locale = raw_input('''{0}> Please type in the locale you want to use: \
{1}'''.format(COLORS['OKBLUE'], COLORS['ENDC']))
            break
...
def setup_bootloader():
    ...
    intelornot = system_output('''cat /proc/cpuinfo | grep -m1 vendor_id |\
awk '{print $NF}' ''')
...
/mnt/boot/EFI/boot/bootx64.efi''')  # Check this
...
def configure_system():
    ...
    if part_type == 2 and uefi:
        system('''echo "/dev/{0}              /boot           vfat         \
rw,relatime,fmask=0022,dmask=0022,codepage=437,\
iocharset=iso8859-1,shortname=mixed,errors=remount-ro        0\
2" > /mnt/etc/fstab'''.format(BOOT))
    elif part_type == 2:
        system('''echo "/dev/{0}              /boot           {1}         \
defaults        0       2" > /mnt/etc/fstab'''.format(BOOT, fs))

    if part_type == 2:
        system('''echo "/dev/mapper/root        /               {0}         \
defaults        0       1" >> /mnt/etc/fstab'''.format(fs))
...
/mnt{0}'''.format(pacmanconf))
    ...
    if cpubits == '64':
        print_info('''We have detected you are running x86_64. \
It is advised to enable multilib with the ArchStrike repo. \
Do you want to enable multilib? (say no if it's already enabled)''')
        if query_yes_no(">", 'yes'):
...
Include = /etc/pacman.d/archstrike-mirrorlist|' /mnt{0}'''.format(pacmanconf))
...
def add_user():
    ...
    if query_yes_no("> Would you like to add a new user?", 'yes'):
        while not username:
            username = raw_input('''{0}> Please enter a username: \
{1}'''.format(COLORS['OKBLUE'], COLORS['ENDC']))
            logger.log(logging.INFO, "Adding user {0}".format(username))
...
def set_video_utils(user):  # noqa
    ...
    if query_yes_no("> Would you like to set up video utilities?", 'yes'):
        while True:
            ...
            options.sort()
            for k in options:
                print_info('{0}) {1}'.format(k, gpus[k]))
            gpu = raw_input('''{0}> Choose an option or leave empty for default: \
{1}'''.format(COLORS['OKBLUE'], COLORS['ENDC'])) or '5'
            try:
                sel = gpus[gpu]
                logger.log(logging.INFO, "Video Utils: {0}".format(sel))
                system('''pacman -S xorg-server xorg-server-utils xorg-xinit xterm \
{0} --noconfirm'''.format(sel), True)
                break
            except KeyError:
...
Window Manager?''', 'yes'):
        ...
        while not opt:
            ...
            4) All
            """)
            opt = raw_input("{0}> Choice: {1}".format(COLORS['OKBLUE'],
                                                      COLORS['ENDC']))
            if opt == '4':
                opt = '123'
...
/mnt/home/{0}/.xinitrc'''.format(username))
                system('''cp -a /mnt/usr/share/archstrike-openbox-config/etc/* \
/mnt/home/{0}/.config/'''.format(username))
                system('''chown {0}:users -R /home/{0}/.config \
...
/mnt/home/{0}/.xinitrc'''.format(username))
                system('''cp -a /mnt/usr/share/archstrike-xfce-config/config/* \
/mnt/home/{0}/.config/'''.format(username))
                system('''chown {0}:users -R /home/{0}/.config \
/home/{0}/.xinitrc'''.format(username), True)
            ...
            system('''cp -a /mnt/usr/share/archstrike-xfce-config/config/* \
/mnt/root/.config/''')
            system('''cp -a /mnt/usr/share/archstrike-xfce-config/icons/* \
/mnt/usr/share/pixmaps/''')
            system('''cp -a /mnt/usr/share/archstrike-xfce-config/wallpapers/* \
/mnt/usr/share/backgrounds/xfce/''')
        ...
        if '3' in opt:
            ...
            if username:
                ...
                system('''echo 'exec i3' > \
/mnt/home/{0}/.xinitrc'''.format(username))
                system('''cp -a /mnt/usr/share/archstrike-i3-config/config/* \
/mnt/home/{0}/.config/'''.format(username))
                system('''cat /mnt/usr/share/archstrike-i3-config/Xresources >> \
/mnt/home/{0}/.Xresources'''.format(username))
                system('''cp -a /mnt/usr/share/archstrike-i3-config/gtkrc-2.0 \
/mnt/home/{0}/.gtkrc-2.0'''.format(username))
                system('''chown {0}:users -R /home/{0}/.config \
/home/{0}/.xinitrc /home/{0}/.Xresources \
